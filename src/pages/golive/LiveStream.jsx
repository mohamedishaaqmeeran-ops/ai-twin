...
export default function LiveStream() {
  ...
  const twinImage = localStorage.getItem("twinImage") || "/images/girl.png";
  const twinName = localStorage.getItem("twinName") || "My AI Twin";

  const liveSetup = JSON.parse(localStorage.getItem("liveSetup") || "{}");
  const product = liveSetup.product || localStorage.getItem("selectedProduct") || "Vitamin C Glow Serum";
  const platforms = liveSetup.platforms || JSON.parse(localStorage.getItem("selectedPlatforms") || '["Instagram"]');
    ...
    <div className="grid gap-6 xl:grid-cols-[1fr_380px]">
      ...
      <section className="relative min-h-[720px] overflow-hidden rounded-3xl border border-border bg-[#0d0d12] shadow-sm">
        {cameraOn ? (
          ...
          <div className="grid h-[720px] place-items-center text-white">
            <div className="text-center">
              ...
              <p className="mt-3 font-black">Camera Off</p>
            </div>
          </div>
        )}
        ...
        <div className="absolute left-5 top-5 flex flex-wrap gap-3">
          <span className="rounded-[5px] bg-red-600 px-4 py-2 text-sm font-black text-white">
            LIVE
          </span>

          <span className="rounded-[5px] bg-black/60 px-4 py-2 text-sm font-black text-white backdrop-blur">
            👁 {formattedViewers}
          </span>
          ...
          {platforms.map((item) => (
            <span
              key={item}
              className="rounded-[5px] bg-white/15 px-4 py-2 text-sm font-bold text-white backdrop-blur"
            >
              {item}
            </span>
          ))}
        ...
        <div className="absolute left-5 top-24 max-w-md rounded-2xl bg-white/90 p-4 backdrop-blur">
          <p className="text-sm font-black brand-text">{twinName}</p>
          <p className="mt-1 text-sm text-gray-700">
            Today I’m showing you why {product} is perfect for your daily routine.
        ...
        <div className="absolute left-5 top-56 max-w-md space-y-3">
          {comments.slice(0, 4).map((msg, index) => (
            <div
              key={`${msg}-${index}`}
              className="rounded-2xl bg-black/60 px-4 py-3 text-sm text-white backdrop-blur"
            >
        ...
        {/* Product Card */}
        {productVisible && (
          <div className="absolute bottom-5 left-5 right-5 flex flex-col gap-4 rounded-2xl bg-white p-4 shadow-xl sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="font-black brand-text">₹799</p>
            ...
            <button className="brand-gradient rounded-[5px] px-6 py-3 text-sm font-bold text-white">
              Buy Now
            </button>
          </div>
        )}
        ...
        {ended && (
          <div className="absolute inset-0 grid place-items-center bg-black/70">
            <div className="rounded-3xl bg-white p-8 text-center">
              ...
              <h2 className="mt-4 text-2xl font-black brand-text">
                Live Ended
              </h2>
      ...
      <aside className="space-y-6">
        <div className="rounded-3xl border border-border bg-card p-5 shadow-sm sm:p-6">
          <h2 className="text-xl font-black brand-text">Live Controls</h2>
        ...
        <div className="rounded-3xl border border-border bg-card p-5 shadow-sm sm:p-6">
          <h2 className="text-xl font-black brand-text">Live Stats</h2>
        ...
        <div className="rounded-3xl border border-border bg-card p-5 shadow-sm sm:p-6">
          <h2 className="text-xl font-black brand-text">Send Chat Prompt</h2>
        ...
        <button
          onClick={endLive}
          className="w-full rounded-[5px] bg-red-600 py-3 text-sm font-bold text-white hover:bg-red-700"
        >
          End Live
        </button>
      </aside>
...
function Control({ icon: Icon, text, active, onClick }) {
  return (
    <button
      onClick={onClick}
      className={`rounded-2xl border p-5 text-center font-bold transition hover:-translate-y-1 hover:shadow-md ${
        active
          ? "border-[var(--brand-pink)] bg-pink-50 text-[var(--brand-pink)]"
          : "border-border bg-background text-muted-foreground"
      }`}
    >
      <div className="mx-auto mb-2 grid h-10 w-10 place-items-center rounded-xl bg-white text-[var(--brand-pink)]">
